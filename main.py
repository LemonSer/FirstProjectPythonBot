...
async def skills_1(callback_query: types.CallbackQuery):
    await bot.send_photo(
        ...
        photo='https://static.wikia.nocookie.net/leagueoflegends/images/2/2e/Mordekaiser_Darkness_Rise.png/revision/latest?cb=20190527181220',
        caption='Наносящие урон автоатаки и умения Мордекайзера против вражеских чемпионов и больших монстров дают ему заряд на 4 секунды. При трех зарядах, Мордекайзер получает эффект Восхождение тьмы. Длительность эффекта обновляется при последующем нападении на чемпионов и монстров.',
        reply_markup=skills()
    )
    # Удаление исходного сообщения, которое вызвало callback_query
